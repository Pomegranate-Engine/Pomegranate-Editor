...
Notification::Notification(Texture *icon, const std::string& title, const std::string& content, float lifetime)
{
    this->icon = icon;
    this->title = title;
...
void Notify::render(float dt)
{
    for (int i = notifications.size()-1; i>0 ; --i)
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_WindowBg,ImVec4(EditorTheme::editor_popups.x/255.0f,EditorTheme::editor_popups.y/255.0f,EditorTheme::editor_popups.z/255.0f, alpha));
